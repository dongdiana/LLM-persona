...
def run_in_batches_and_save(segment_rows: List[Dict], keyword: str, mode: str,
    ...
    # NEW: 누락분 재생성 → part_101.json
    if recovery_entries:
        logger.info(f"[recovery] total missing entries: {len(recovery_entries)} → re-generate into part_101")
        retry_prompts = make_prompt_from_entries(recovery_entries, keyword, mode, context_dir)
        if log_prompts:
            (dbg_dir / f"batch_101.system.txt").write_text(retry_prompts["system"], encoding="utf-8")
            (dbg_dir / f"batch_101.user.txt").write_text(retry_prompts["user"], encoding="utf-8")
        ...
        patch_path = log_dir / f"personas_{keyword}_part_101.json"
        patch_path.write_text(json.dumps(data2 if data2 is not None else {"raw_text": text2},
                                         ensure_ascii=False, indent=2), encoding="utf-8")
